...
knit: (function(inputFile, encoding) {
  ...
  rmarkdown::render(inputFile,
      encoding=encoding, 
      output_file=file.path(dirname('C:\\Users\\psatorra\\Documents\\TFM'),'TFM/5_Productes/Analysis_Spatial_Temporal_Vaccination_v2.html')) })
output:
  html_document:
    toc: true
    toc_float:
...
#dat for the population 
load(file.path(dades_ana, "dat_70.Rda"))
...
load(file.path(dades_ana, "dat_vac.Rda"))

#Load covariates
load(file = file.path(dades_ana, "Covariates/dat_covar.Rda"))

#Vaccination effect
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_raw.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_vac.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_vac_lag2.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_70.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_vac_nocovar.Rda"))
```

# Description

We have fully vaccination data since 2020-28-12 but until the week of 2021-01-17 we don't have enough fully vaccination counts (only 17 fully vaccination individuals). Also we won't take the data of the last day, 2022-07-25 as it's outside the study period (2022-07-24). Therefore, we will take the vaccination data from 2021-01-17 to 2022-07-24.

Fully vaccination is defined as administrations of second doses or administrations of one shot (J&J) vaccines. For the analysis, we will consider the cumulative percentage of fully vaccination individuals as weekly rates don't take in account previous history of vaccines administered. Because administered vaccines in a given time point can have an effect in posterior time outcomes,we have to take in account all the entire vaccination history in that area. 

`r tab_nums("tab_vac_cum", "Descriptive table of the cumulative fully vaccination counts and percentage of all ABS in the whole period")`
...
dat %>% 
  ...
  tbl_summary(
    label = list(n_vac ~ "ABS cumulative fully vaccination counts",
                 rate_vac ~ "ABS cumulative fully vaccination percentage (%)"),
...
`r fig_nums("plot_vac", "Plot of the evolution of the weekly fully vaccination rate per 100k habitants across the study period")`
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  ylab("Weekly vaccination rate (x100k hab.)")
```

The 5th wave is were daily rates of fully vaccination have their peak.
...
ggplot(data = sdat_cat, aes(x = data, y = Trate)) +
  ...
  theme_classic() + 
  theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
  ylab("Cumulative vaccination percentage (%)")
```

`r fig_nums("map_vac", "Map of the total cumulative fully vaccination percentage across all the study period for each ABS")`
...
cat <- ggplot(map) + 
       ...
       # ggtitle("Cumulative fully vaccination percentage") +
...
Let's see the distribution of the cumulative fully vaccination percentage by age and sex groups.

`r fig_nums("sex_vac", "Cumulative fully vaccination percentage in the whole period by sex groups")`
...
#   geom_area() +
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   ylab("Week fully vaccination rate (x100k hab.)") +
#   xlab("") +
#   theme_minimal() +
...
In the beginning of the vaccination campaign the percentage of fully vaccination in women was higher than in men while in the 4th wave it was compensated a bit. This might happen because there is more old women than old men and the first vaccinated population were the oldest one.

`r fig_nums("age_vac_ts", "Evolution of the daily fully vaccination rate in function of the age group (bigger or lower than 70 years)")`
...
#   geom_area() +
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   ylab("Daily vaccination rate (x100k hab.)") +
#   xlab("") +
#   theme_minimal() +
...
In the first weeks of the vaccination campaign the fully vaccination percentage of the oldest group is a lot bigger than in the youngest group (3th-4th wave) as they were vaccination before the younger population. In fact, from the 5th wave the rate of fully vaccination of people older than 70 years old is nearly zero.

`r fig_nums("vac_urban", "Boxplot of cumulated fully vaccination coverage in function of urban/rural areas")`
...
ggplot(sdat, aes(x = urban, y = rate_vac, fill = urban)) + 
        ...
        ylab("Cumulative fully vaccinations %") +
...
Finally, as we have seen the majority of people with 70 years old or more are vaccinated in the 3rd-4th wave. Thus, in this period we will study the effect of the fully vaccination on the hospitalization outcome only taking in account the population older than 70 years old. In this group of age it is believed that vaccination has a bigger effect.
...
First, let's illustrate the differences in the cumulative fully vaccination percentage in all the period between the hot/cold spots estimated by the raw model on the hospitalization outcome.
...
Hot spots have lower fully vaccination percentages than cold spots, so we might think that the vaccination could explain some of the spatial effect.

Let's show the differences on the cumulative fully vaccination percentage of the previous week between the estimated hot/cold spots of the actual week, for all the weeks of the study period.
...
ggplot(plot_vac, aes(x = data, y = lag_rate1*100, fill = p_cat, group = group)) + 
  ...
  ylab("Lagged fully vaccination (%)") +
  scale_fill_manual(name = "", values = c("#42e7e9", "#bd1816")) +
  theme_minimal()
```

We can't see clear differences on the lagged cumulative fully vaccination percentage between hot and cold spots.
...
It seems that across all period hot spots have systematically lower lagged cumulative values of fully vaccination percentages than cold spots.
  ...
  mutate(
    hp = map2(model, res, function(x, y) {
      est <- summary(y)$fixed %>% 
        ...
        mutate(
          var = factor(var, levels = c("(Intercept)", "lag_rate_vac1", "urbanUrban", "isc", "lag_rate_vac2"), labels = c("(Intercept)", "Fully vaccination (1-week lag)", "Urban vs Rural", "Socioeconomic Index (SI)", "Fully vaccination (2-week lags)"))
        ) %>% 
        mutate(
          order = 1
        )
...
First, let's illustrate the differences in the cumulative fully vaccination percentages in all the period between hospitalization hot/cold spots estimated by the raw model.
...
In the third wave, spatio-temporal hot spots have lower percentages of the lagged fully vaccination, meanwhile in the four wave differences are not clear.
...
The vaccination has a protective effect over the SIR of cases (5%) when adjusted by covariates. The effect disappears considerin 2-week lags.

The role of the structural spatial effect increases a little bit when adjusting by the vaccination and the covariates.
  ...
  mutate(
    hp = map2(model, res, function(x, y) {
      est <- summary(y)$fixed %>% 
        ...
        rownames_to_column(var = "var") %>% 
        mutate(
          var = factor(var, levels = c("(Intercept)", "lag_rate_vac1", "urbanUrban", "isc", "lag_rate_vac2"), labels = c("(Intercept)", "Fully vaccination (1-week lag)", "Urban vs Rural", "Socioeconomic Index (SI)", "Fully vaccination (2-week lag)"))
        ) %>% 
        mutate(
          order = 1
        )
...
The vaccination effect is at limit of the significance (3% alone and 2% when adjusting by covariates).
...
Vaccination has a protective effect of 16%. Considering 2-week lags this effect disappears.
...
The spatial variance increases in detriment of the temporal variance. The spatio-temporal variance also increases.
...
The vaccination effect has a protective effect of the 14% adjusted by covariates. Considering 2-week lags the effect is very small at limit of significance.
  ...
  mutate(
    hp = map(res, function(x) {
      ...
      est <- summary(x)$fixed %>% 
        ...
        rownames_to_column(var = "var") %>% 
        mutate(
          var = factor(var, levels = c("(Intercept)", "lag_rate_vac1", "urbanUrban", "isc"), labels = c("(Intercept)", "Fully vaccination (1-week lag)", "Urban vs Rural", "Socioeconomic Index (SI)"))
        ) %>% 
        mutate(
          order = 1
        )
...
Now, the vaccination has a protective effect of 5% very close to significance. Urban areas have a risk effect of 27% and SI index a risk effect of 15%.
...
The vaccination has a bigger protective effect of 27% over hospitalizations. The SI index has a risk effect of 15% and the urban areas a risk effect of 25%.
...
Hot spots have systematically lower cumulated values of fully vaccination percentages than cold spots.
...
The percentage values are very similar.
