...
knit: (function(inputFile, encoding) {
  ...
  rmarkdown::render(inputFile,
      encoding=encoding, 
      output_file=file.path(dirname('C:\\Users\\psatorra\\Documents\\TFM'),'TFM/5_Productes/Analysis_Spatial_Temporal_Vaccination_ve.html')) })
output:
  html_document:
    toc: true
    toc_float:
...
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_70_raw.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_70_lag2.Rda"))
load(file = file.path(dades_ana, "SpatioTemporal/Vaccination/ndat_sae_vac_nocovar.Rda"))
```

# Description

We have full vaccination data since 2020-28-12 but until the week of 2021-01-17 we don't have enough full vaccination counts (only 17 full vaccination individuals). Also we won't take the data of the last day, 2022-07-25 as it's outside the study period (2022-07-24). Therefore, we will take the vaccination data from 2021-01-17 to 2022-07-24.
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  ylab("Weekly vaccination rate (x100k pop.)")

# png(r"(C:\Users\psatorra\Documents\TFM\5_Productes\Figures\evo_vac_weekly.png)",width = 4000,height = 1800,res = 600)
# ggplot(data = sdat_cat, aes(x = data, y = rate)) +
#   geom_bar(stat = "identity",fill="#DD8888", alpha = 0.5)+
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   geom_vline(aes(xintercept = xint), linetype = "dashed") +
...
# png(r"(C:\Users\psatorra\Documents\TFM\5_Productes\Figures\evo_vac_cum.png)",width = 4000,height = 1800,res = 600)
# ggplot(data = sdat_cat, aes(x = data, y = Trate)) +
#   geom_bar(stat = "identity",fill="#DD8888", alpha = 0.5)+
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   geom_vline(aes(xintercept = xint), linetype = "dashed") +
...
staula <- sdat_sae %>%
  ...
  dplyr::select(model, dic, waic) %>%
  dplyr::select(model, dic, waic) %>%
  filter(model %in% c("Raw", "Adjusted"))
