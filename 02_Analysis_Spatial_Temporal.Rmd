...
knit: (function(inputFile, encoding) {
  ...
  rmarkdown::render(inputFile,
      encoding=encoding, 
      output_file=file.path(dirname('C:\\Users\\psatorra\\Documents\\TFM'),'TFM/5_Productes/Analysis_Spatial_Temporal_v3.html')) })
output:
  html_document:
    toc: true
    toc_float:
...
load(file = file.path(dades_ana, "SpatioTemporal/Covariates/ndat_sae_si.Rda"))
...
nmap <- var_lab %>% 
  mutate(
    map = pmap(list(var, lab, lab_legend), function(x, y, z) {
      ...
      if(grepl("_cat$", x)) {
        # Categoric variable  
        cat <-  cat + scale_fill_viridis(name = z, option = "magma", direction = -1, discrete = TRUE) 
        bcn <- bcn + scale_fill_viridis(name = z, option = "magma", direction = -1, discrete = TRUE, guide = "none") 
      } else if(x == "urban") {
        ...
        #Continuous variable
        cat <-  cat + scale_fill_viridis(name = z, option = "magma", direction = -1) 
        bcn <-  bcn + scale_fill_viridis(name = z, option = "magma", direction = -1, guide = "none") 
      }
  ...
  mutate(
    map = pmap(list(var, lab, lab_legend), function(x, y, z) {
      ...
      cat <- ggplot(dat_map) + 
            ...
            ggtitle(y) + 
            scale_fill_gradient2(name = z, midpoint = 1, low = "blue", mid = "white", high = "red") 
      ...
      bcn <- ggplot(dat_map) + 
            ...
            scale_fill_gradient2(name = z, midpoint = 1, low = "blue", mid = "white", high = "red", guide = "none") 
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  ylab("7-day case rate (x100k hab.)")
```
...
`r fig_nums("plot_cas_rs", "Plot of the weekly case incidence by each of the health regions along with the median across time")`
...
`r fig_nums("sex_cas_ts", "Evolution of the daily case rate in function of the sex group")`
...
`r fig_nums("age_cas_ts", "Evolution of the daily case rate in function of the age group (bigger or lower than 70 years)")`
...
#   geom_area() +
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   ylab("Daily case rate (x100k hab.)") +
#   xlab("") +
#   theme_minimal() +
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  theme_classic() + 
  theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
  ylab("Weekly hospitalisation rate (x100k hab.)")
```
...
ggplot(plot_hosp, aes(x = Tratio * 100, y = edat, fill = sexe)) +
  geom_col() +
  scale_x_continuous(breaks = seq(-100, 100, by = 2.5), labels = abs(seq(-100, 100, by = 2.5))) +
  expand_limits(x = c(-12.5, 12.5)) +
  xlab("%") +
  ylab("Age") +
  theme_minimal() +
  scale_fill_brewer(palette = "Set2", name = "", labels = c("Women", "Men")) 
...
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
#   ylab("Week hospitalisation rate (x100k hab.)") +
#   xlab("") +
#   theme_minimal() +
#   scale_fill_brewer(palette = "Set2", name = "", labels = c("Men", "Women"), direction = -1) 
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  scale_y_continuous(name = "hospitalisation rate", sec.axis = sec_axis(~./50, name = "Phi", breaks = c(0, 0.25, 0.5, 0.75, 1)))
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  scale_y_continuous(name = "Case rate", sec.axis = sec_axis(~./3200, name = "SD"))
...
ggplot(data = sdat_cat, aes(x = data, y = rate)) +
  ...
  scale_y_continuous(name = "hospitalisation rate", sec.axis = sec_axis(~./40, name = "SD"))
...
bcn <- ggplot(map) + 
       ...
       scale_fill_gradient2(
        midpoint = 1, low = "blue", mid = "white", high = "red", guide = "none"
       ) +
       coord_sf(
         xlim = c(2.1, 2.25),
         ylim = c(41.32, 41.475)
...
p1 <- ggplot() +
  ...
  scale_y_continuous(name = "Case rate", sec.axis = sec_axis(~./150, name = "Temporal IRR"))
...
p2 <- ggplot() +
  ...
  scale_y_continuous(name = "hospitalisation rate", sec.axis = sec_axis(~./14, name = "Temporal IRR"))
...
# ggplotly(ggplot(data = srr_sp_temp, aes(x = data, y = rr, group = factor(idarea), color = factor(idarea), text = abs)) +
#   geom_line(alpha = 0.3) +
#   geom_hline(yintercept = 1, color = "black", linetype = "dashed") +
#   scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m") +
...
#   theme_classic() + 
#   theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
#   ggtitle("Cases"), tooltip = c("data", "text"))
# 
...
#   theme_classic() + 
#   theme(axis.line=element_blank(),axis.title.x=element_blank(), axis.ticks.y = element_blank()) +
#   ggtitle("hospitalisation"), tooltip = c("data", "text"))
```
...
nplot <- rbind(var_lab %>% filter(var == "isc"), var_lab2) %>% 
  mutate(
...
Urban areas have a risk effect of the 7%. The SI index have a close to significance protector effect of 1%. The effect doesn't change much between rural and urban areas. The other SI component that doesn't have a non-significant effect is the income that has a protective effect of the 3%. 
...
Let's plot the posterior estimates of the model adjusted by SI.

```{r echo=FALSE, warning=FALSE, message=FALSE}
sdat_sae <- ndat_sae_si %>% 
  filter(effect == "sir2")
```
...
Let's plot the posterior estimates of the model adjusted by urban and SI index, that is the best adjusted model for the hospitalisation outcome in terms of the DIC/WAIC.
