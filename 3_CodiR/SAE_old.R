...
Tpob_cas <- pob_abs %>% 
  group_by(sexe, edat_cas) %>% 
  summarise(NT = sum(N)) %>% 
  rename(edat = edat_cas)

Tpob_hosp <- pob_abs %>% 
  group_by(sexe, edat_hosp) %>% 
  summarise(NT = sum(N)) %>% 
  rename(edat = edat_hosp)

Tpob_vac <- pob_abs %>%
  group_by(sexe, edat_vac) %>%
  summarise(NT = sum(N)) %>%
...
res_out <- function(x, inla = TRUE){
  ...
  Tedat_cas <- dat_edat %>% 
    ...
    group_by(sexe, edat) %>% 
    summarise(n = sum(n)) %>% 
    full_join(Tpob_cas, by = c("sexe", "edat")) %>% 
    mutate(
  ...
  Tedat_hosp <- dat_hosp %>% 
    ...
    group_by(sexe, edat) %>% 
    summarise(n = sum(n)) %>% 
    full_join(Tpob_hosp, by = c("sexe", "edat")) %>% 
    mutate(
      n = ifelse(is.na(n), 0, n),
      ratio = n/NT
    )
  ...
  #Fully vaccination cumulative rate:
  Tedat_vac <- dat_vac %>%
    filter(data <= x) %>%
    group_by(sexe, edat) %>%
    summarise(n = sum(n)) %>%
    full_join(Tpob_vac, by = c("sexe", "edat")) %>%
    mutate(
      n = ifelse(is.na(n), 0, n),
      ratio = n/NT
    )
  ...
  exp_vac <- pob_abs %>%
    ...
    group_by(codi_abs, sexe, edat) %>% 
    summarise(N = sum(N)) %>% 
    left_join(Tedat_vac, by = c("sexe", "edat")) %>%
    mutate(
      exp = N*ratio
    ) %>%
    group_by(codi_abs) %>%
  ...
  return(inla_res)
    ...
    #We want to return only the observed SIR and taxes for the date (without the estimated models)
    return(map@data)
...
dat <- tibble(data = seq(range_data[1], range_data[2], by = 1)) %>%
  ...
  dplyr::select(-wday) %>%
  mutate(
    obs = map(data, ~res_out(.x, inla = FALSE))
  ) %>% 
  unnest(obs) %>% 
  dplyr::select(data, codi_abs, abs, idarea, N:sir_vac)

#Lag vaccination data
dat <- dat %>% 
  group_by(codi_abs) %>% 
  mutate(
    lag_rate_vac = lag(rate_vac),
    lag_sir_vac = lag(sir_vac)
  ) %>% 
  ungroup()
...
inla_mod_st <- function(df, outcome = "cas", model = "bym2", iid = FALSE, rw = "rw1", interaction = "no", effect = "sir") {
  ...
  if (model == "bym2") {
    if (!iid) {
      ...
      #This is the base model we will use. Define the models with the different interactions terms:
      if (interaction == "no") {
        formula <-
          n ~ f(
            idarea,
    ...
    formula = n ~ f(idarea, model = "bym", graph = g, hyper = list(prec.unstruct = list(prior = sdunif), prec.spatial = list(prior = sdunif)), constr = TRUE) +
        f(idtime, model = rw, hyper=list(prec=list(prior=sdunif)), constr = TRUE)
    }
    ...
    if(effect == "sir") {
      ...
      #INLA model for the smooth incidence tax
      mod <- inla(formula, family="poisson", data=df, E=N, control.compute=list(dic = TRUE, cpo = TRUE, waic = TRUE), control.predictor=list(compute=TRUE))
    }
...
#We will compare different base models to see which model specify:
t0 <- Sys.time()
#Comparison between BYM and BYM2 models:
ndat_sae_st_bym <- tibble(expand.grid(outcomes = c("cas", "hosp"), model = c("bym2", "bym"))) %>% 
  mutate_all(as.character) %>% 
  mutate(
    res = pmap(list(outcomes, model), function(x, y) {
      inla_mod_st(df = dat, outcome = x, model = y)
    }),
...
ndat_sae_st_iid <- tibble(expand.grid(outcomes = c("cas", "hosp"), iid = c(FALSE, TRUE))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = pmap(list(outcomes, iid), function(x, y) {
      inla_mod_st(df = dat, outcome = x, iid = y)
    }),
    dic = map_dbl(res, ~.x$dic$dic),
    waic = map_dbl(res, ~.x$waic$waic)
  )
...
ndat_sae_st_rw <- tibble(expand.grid(outcomes = c("cas", "hosp"), rw = c("rw1", "rw2"))) %>% 
  mutate_all(as.character) %>% 
  mutate(
    res = pmap(list(outcomes, rw), function(x, y) {
      inla_mod_st(df = dat, outcome = x, rw = y)
    }),
    dic = map_dbl(res, ~.x$dic$dic),
    waic = map_dbl(res, ~.x$waic$waic)
  )
...
st_dic_waic_base <- ndat_sae_st_base %>% 
  dplyr::select(outcomes, base, dic, waic) %>% 
  ...
  dplyr::select(base, dic_cas, waic_cas, dic_hosp, waic_hosp)
...
t0 <- Sys.time()
ndat_sae_st1 <- tibble(expand.grid(outcomes = c("cas", "hosp"), interaction = c("no", "I"))) %>% 
  mutate_all(as.character) %>% 
  mutate(
...
#Let's fit the type II model without constraints to see the change, and also for the tax
...
#For the tax
ndat_sae_II_tax <- tibble(expand.grid(outcomes = c("cas", "hosp"), effect = c("tax"))) %>%
  mutate_if(is.factor, as.character) %>%
  mutate(
    res = map2(outcomes, effect, ~inla_mod_II(dat, outcome = .x, effect = .y))
  )

ndat_sae_II <- rbind(ndat_sae_II, ndat_sae_II_tax)
...
ndat_sae_nocons <- tibble(outcomes = "cas", effect = c("sir", "tax")) %>% 
  mutate(
...
#We will pass the defined parameters to the function directly
inla_mod_covar <- function(df, outcome = "cas", effect = "sir") {
  ...
  df <- df %>% 
    mutate_at(c("perc_65", "dens", "index_socioeconomic"), scale)
  
  if(effect == "sir") {
    formula <- n ~ dens + index_socioeconomic + 
    ...
    formula <- n ~ perc_65 + dens + index_socioeconomic + 
      f(idarea, model = "bym2", graph = g, hyper = pc_prior, constr = TRUE) +
      f(idtime, model = "rw1", hyper=list(prec=list(prior=sdunif)), constr = TRUE) +
      f(idareatime,
        model = "generic0", Cmatrix=R, rankdef=r,
...
ndat_sae_covar <- tibble(expand.grid(outcomes = c("cas", "hosp"), effect = c("sir", "tax"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(outcomes, effect, ~inla_mod_covar(dat, outcome = .x, effect = .y))
  )

#Save the model
save(ndat_sae_covar, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_covar.Rda"))

#Second model with covariates adjusting by the different components of the socieconomic index

#We will pass the defined parameters to the function directly
inla_mod_covar2 <- function(df, outcome = "cas", effect = "sir") {
  ...
  if(effect == "sir") {
    formula <- n ~ dens + poblacio_exempta_de_copagament_farmaceutic + poblacio_amb_rendes_inferiors_a_18_000_euros + poblacio_amb_nivell_dinstruccio_insuficient + taxa_de_mortalitat_prematura + hospitalitzacions_evitables +
      f(idarea, model = "bym2", graph = g, hyper = pc_prior, constr = TRUE) +
      f(idtime, model = "rw1", hyper=list(prec=list(prior=sdunif)), constr = TRUE) +
      f(idareatime,
        model = "generic0", Cmatrix=R, rankdef=r,
    ...
    formula <- n ~ perc_65 + dens + poblacio_exempta_de_copagament_farmaceutic + poblacio_amb_rendes_inferiors_a_18_000_euros + poblacio_amb_nivell_dinstruccio_insuficient + taxa_de_mortalitat_prematura + hospitalitzacions_evitables +
      f(idarea, model = "bym2", graph = g, hyper = pc_prior, constr = TRUE) +
      f(idtime, model = "rw1", hyper=list(prec=list(prior=sdunif)), constr = TRUE) +
      f(idareatime,
        model = "generic0", Cmatrix=R, rankdef=r,
...
ndat_sae_covar2 <- tibble(expand.grid(outcomes = c("cas", "hosp"), effect = c("sir", "tax"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(outcomes, effect, ~inla_mod_covar2(dat, outcome = .x, effect = .y))
  )

#Save the model
save(ndat_sae_covar2, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_covar2.Rda"))
...
#We will pass the defined parameters to the function directly
inla_mod_vac <- function(df, outcome = "cas", effect = "sir", lag = TRUE, vac = TRUE, sir = FALSE, covar = TRUE) {
  ...
  #Scale the covariates
  df <- df %>% 
    mutate_at(c("perc_65", "dens", "index_socioeconomic", "rate_vac", "lag_rate_vac", "lag_sir_vac"), scale)
  ...
  covar_mod <- case_when(
    covar ~ str_glue("{perc_65}dens + index_socioeconomic +"),
    TRUE ~  ""
  )
  
  vac_mod <- case_when(
    vac ~ str_glue("{lag_vac}{sir_vac} +"),
    TRUE ~ ""
  )
...
#We have fully vaccinated data since 2020-28-12 but before 2021-01-17 there're very few cases. We take one week more to account for the lags.
...
sdat$idtime <- as.numeric(factor(sdat$data))
sdat$idtime1 <- sdat$idtime
sdat$idareatime <- 1:nrow(sdat)

#First period: 3rd and 4th wave
sdat1 <- dat %>% 
  filter(data >= ymd("2021-01-24"), data < ymd("2021-06-13")) %>% 
  mutate(
    rate_vac = rate_vac*100
  )

sdat1$idtime <- as.numeric(factor(sdat1$data))
sdat1$idtime1 <- sdat1$idtime
sdat1$idareatime <- 1:nrow(sdat1)

#Second period: 5th wave
sdat2 <- dat %>% 
  filter(data >= ymd("2021-06-13"), data < ymd("2021-11-02")) %>% 
  mutate(
    rate_vac = rate_vac*100
  )

sdat2$idtime <- as.numeric(factor(sdat2$data))
sdat2$idtime1 <- sdat2$idtime
sdat2$idareatime <- 1:nrow(sdat2)

#Third period: 6th wave
sdat3 <- dat %>% 
  filter(data >= ymd("2021-11-02")) %>% 
  mutate(
    rate_vac = rate_vac*100
  )

sdat3$idtime <- as.numeric(factor(sdat3$data))
sdat3$idtime1 <- sdat3$idtime
sdat3$idareatime <- 1:nrow(sdat3)

ndat_sae_vac <- tibble(expand.grid(period = c("All", "3rd-4th wave", "5th wave", "6th wave"), outcomes = c("cas", "hosp"), effect = c("sir"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(period, outcomes, function(x, y) {
      if(x == "3rd-4th wave") {
        inla_mod_vac(df = sdat1, outcome = y, effect = "sir")
      } else if(x == "5th wave"){
        inla_mod_vac(df = sdat2, outcome = y, effect = "sir")
      } else if(x == "6th wave") {
        inla_mod_vac(df = sdat3, outcome = y, effect = "sir")
      } else {
        inla_mod_vac(df = sdat, outcome = y, effect = "sir")
      }
    })
  )

#Save the model
save(ndat_sae_vac, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_vac.Rda"))

#Let's estimate the raw model:
ndat_sae_raw <- tibble(expand.grid(period = c("All", "3rd-4th wave", "5th wave", "6th wave"), outcomes = c("cas", "hosp"), effect = c("sir"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(period, outcomes, function(x, y) {
      if(x == "3rd-4th wave") {
        inla_mod_vac(df = sdat1, outcome = y, effect = "sir", vac = FALSE, covar = FALSE)
      } else if(x == "5th wave"){
        inla_mod_vac(df = sdat2, outcome = y, effect = "sir", vac = FALSE, covar = FALSE)
      } else if(x == "6th wave") {
        inla_mod_vac(df = sdat3, outcome = y, effect = "sir", vac = FALSE, covar = FALSE)
      } else {
        inla_mod_vac(df = sdat, outcome = y, effect = "sir", vac = FALSE, covar = FALSE)
      }
    })
  )

#Save the model
save(ndat_sae_raw, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_raw.Rda"))

#Let's estimate now the tax and see if there is some changes in the vaccination effect of interest:
ndat_sae_vac_tax <- tibble(expand.grid(period = c("3rd-4th wave", "5th wave", "6th wave"), outcomes = c("cas", "hosp"), effect = c("tax"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(period, outcomes, function(x, y) {
      if(x == "3rd-4th wave") {
        inla_mod_vac(df = sdat1, outcome = y, effect = "tax") 
      } else if(x == "5th wave") {
        inla_mod_vac(df = sdat2, outcome = y, effect = "tax") 
        } else {
        inla_mod_vac(df = sdat3, outcome = y, effect = "tax")
      }
    })
  )
save(ndat_sae_vac_tax, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_vac_tax.Rda"))

ndat_sae_vac_nolag <- tibble(expand.grid(period = c("All", "3rd-4th wave", "5th wave", "6th wave"), outcomes = c("cas", "hosp"), effect = c("sir"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(period, outcomes, function(x, y) {
      if(x == "3rd-4th wave") {
        inla_mod_vac(df = sdat1, outcome = y, effect = "sir", lag = FALSE)
      } else if(x == "5th wave"){
        inla_mod_vac(df = sdat2, outcome = y, effect = "sir", lag = FALSE)
      } else if(x == "6th wave") {
        inla_mod_vac(df = sdat3, outcome = y, effect = "sir", lag = FALSE)
      } else {
        inla_mod_vac(df = sdat, outcome = y, effect = "sir", lag = FALSE)
      }
    })
  )

save(ndat_sae_vac_nolag, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_vac_nolag.Rda"))
...
ndat_sae_vac_nocovar <- tibble(expand.grid(period = c("All", "3rd-4th wave", "5th wave", "6th wave"), outcomes = c("hosp"), effect = c("sir"))) %>% 
  mutate_if(is.factor, as.character) %>% 
  mutate(
    res = map2(period, outcomes, function(x, y) {
      if(x == "3rd-4th wave") {
        inla_mod_vac(df = sdat1, outcome = y, effect = "sir", covar = FALSE)
      } else if(x == "5th wave"){
        inla_mod_vac(df = sdat2, outcome = y, effect = "sir", covar = FALSE)
      } else if(x == "6th wave") {
        inla_mod_vac(df = sdat3, outcome = y, effect = "sir", covar = FALSE)
      } else {
        inla_mod_vac(df = sdat, outcome = y, effect = "sir", covar = FALSE)
      }
    })
  )


save(ndat_sae_vac_nocovar, file = file.path(dades_ana, "SpatioTemporal/ndat_sae_vac_nocovar.Rda"))
